...
gceSTATUS gcQueryShaderCompilerHwCfgWithHardware(
    ...
    OUT PVSC_HW_CONFIG pVscHwCfg)
{
    ...
    /* Call down to the hardware object. */
    status = gcoHARDWARE_QueryShaderCompilerHwCfg(Hardware, pVscHwCfg);
...
gcoSHADER_BindUniform(
    ...
    IN gceUNIFORMCVT Convert,
    IN gcSHADER_KIND Type,
    IN gctINT32 uniformSaveDataType
    )
{
    gceSTATUS status;
    gctUINT32 columns, rows, arrays, matrixStride, arrayStride;
    ...
    status = gcoHARDWARE_BindUniformEx(gcvNULL, Address, Physical, columns, rows, arrays, IsRowMajor,
                                       matrixStride, arrayStride, &Values, Convert, Type, gcvFALSE, uniformSaveDataType);
    ...
    )
{
    ...
    status = gcoHARDWARE_BindUniformEx(gcvNULL, Address, Physical, columns, rows, arrays, IsRowMajor,
                                       matrixStride, arrayStride, Values, Convert, Type, gcvTRUE, uniformSaveDataType);
    ...
    )
{
    ...
    if (size)
    {
        ...
        if (node->pool == gcvPOOL_VIRTUAL || node->pool == gcvPOOL_LOCAL_EXCLUSIVE)
        {
