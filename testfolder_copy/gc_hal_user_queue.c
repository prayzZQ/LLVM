    ...
    )
{
    ...
    gcoHARDWARE_QueryBroCoreMask(Hardware, &queue->broCoreMask);

#if gcdENABLE_SW_PREEMPTION
    queue->priorityID = 0;
    queue->topPriority = gcvFALSE;
#endif
    ...
    )
{
...
#if gcdENABLE_SW_PREEMPTION
    if (gcoHARDWARE_IsEnd(Queue->hardware))
    {
        gcmONERROR(gcoQUEUE_End(Queue));
    }
#endif
    ...
    while (Queue->chunks != gcvNULL)
    {
        ...
        /* Free the memory. */
        gcmONERROR(gcoOS_FreeSharedMemory(gcvNULL, chunk));
    }

    /* Free the queue. */
    gcmONERROR(gcoOS_Free(gcvNULL, Queue));
...
#if gcdENABLE_SW_PREEMPTION
gceSTATUS
gcoQUEUE_SetPriority(
    IN gcoQUEUE Queue,
    IN gctUINT PriorityID,
    IN gctBOOL * TopPriority
    )
{
    gcmHEADER_ARG("Queue=%p PriorityID=%d", Queue, PriorityID);

    gcmVERIFY_ARGUMENT(Queue != gcvNULL);

    Queue->priorityID = PriorityID;

    if (TopPriority)
    {
        Queue->topPriority = *TopPriority;
    }
...
gceSTATUS
gcoQUEUE_GetPriority(
    IN gcoQUEUE Queue,
    OUT gctUINT *PriorityID
    )
{
    ...
    )
{
    ...
    gcmONERROR(gcoHARDWARE_Query3DCoreCount(Queue->hardware, &coreCount));

    if (Queue->priorityID == 0)
    {
        gcmFOOTER_NO();
        return gcvSTATUS_OK;
    }
    ...
    {
        gcmONERROR(gcoHARDWARE_QueryCoreIndex(Queue->hardware, i, &coreIndex));

        gcmONERROR(gcoHAL_SetCoreIndex(gcvNULL, coreIndex));

        iface.command = gcvHAL_COMMIT_DONE;
        iface.u.CommitDone.priorityID = Queue->priorityID;
    ...
    )
{
    ...
    if (Queue->head != gcvNULL)
    {
        ...
        iface.u.Event.broCoreMask = Queue->broCoreMask;

        gcmONERROR(gcoHARDWARE_Query3DCoreCount(Queue->hardware, &coreCount));
        iface.u.Event.shared = coreCount > 1;

#if gcdENABLE_SW_PREEMPTION
        iface.u.Event.priorityID  = Queue->priorityID;
        iface.u.Event.topPriority = Queue->topPriority;
#endif
